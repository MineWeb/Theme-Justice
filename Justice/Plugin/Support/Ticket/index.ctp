  <div class="panel panel-default">
    <div class="panel-heading"><h1 class="panel-title"><i class="fa fa-question"></i> Support</h1></div>
    <div class="panel-body">
      <h4 class="text-center"><i class="fa fa-comments-o"></i> Vous avez besoin d’aide ? Alors n’hésitez pas !</h4>
              ...
              <div class="col-md-12" id="full-ticket-<?= $value['Ticket']['id'] ?>">
                <div class="panel panel-default panel-ticket">
                  <div class="panel-heading">
                    <h1><img class="avatar noresp" src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin/', 'plugin' => false)) ?>/<?= $value['Ticket']['author'] ?>/26" title="<?= $value['Ticket']['author'] ?>">
 <?= $value['Ticket']['author'] ?>
                    ...
                    <a id="<?= $value['Ticket']['id'] ?>" title="<?= $Lang->get('GLOBAL__DELETE') ?>" class="ticket-delete btn btn-danger btn-sm"><icon class="fa fa-times"></icon></a>
                    <?php } ?>
                    <?php if($value['Ticket']['state'] == 0) { ?>
                      ...
                      <a id="<?= $value['Ticket']['id'] ?>" title="<?= $Lang->get('RESOLVED') ?>" class="ticket-resolved btn btn-success btn-sm div-ticket-resolved-<?= $value['Ticket']['id'] ?>"><icon style="font-size: 10px;" class="fa fa-check"></icon></a>
                      <?php } ?>
                    <?php } ?>
                    <?php if($Permissions->can('SHOW_TICKETS_ANWSERS')) { ?>
                        <button id="<?= $value['Ticket']['id'] ?>" title="<?= $Lang->get('SHOW_ANSWER') ?>" class="btn btn-info btn-sm dropdown_reply"><icon style="font-size: 10px;" class="fa fa-chevron-down"></icon></button>
                      <?php } ?>
                      ...
                      <button id="<?= $value['Ticket']['id'] ?>" title="<?= $Lang->get('REPLY') ?>" class="btn btn-warning btn-sm ticket-reply"><icon class="fa fa-mail-reply" style="font-size: 10px;"></icon></button>
                    <?php } ?>
                    </span></h1>
                  </div>
                  <div class="panel-body" style="background-color: #fafafa;">
                    <h3><?= $value['Ticket']['title'] ?> <?php if($value['Ticket']['state'] == 1) { echo '<icon style="color: green;" class="fa fa-check" title="'.$Lang->get('RESOLVED').'"></icon>'; } else { echo '<div style="display:inline-block;" id="ticket-state-'.$value['Ticket']['id'].'"><icon class="fa fa-times" style="color:red;" title="'.$Lang->get('UNRESOLVED').'"></icon></div>'; } ?></h3>
                      ...
                      <?php if($Permissions->can('SHOW_TICKETS_ANWSERS') && !empty($reply_tickets)) { ?>
                        ...
                        <?php foreach ($reply_tickets as $k => $v) { ?>
                          ...
                          <div id="ticket-reply-<?= $v['ReplyTicket']['id'] ?>">
                            <div class="reply-col">
                              <blockquote>
                                  <h4><img class="support avatar noresp" src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin/', 'plugin' => false)) ?>/<?= $v['ReplyTicket']['author']; ?>/32" title="<?= $v['ReplyTicket']['author']; ?>"><?= $v['ReplyTicket']['author']; ?></h4>
  ...
  <div class="panel panel-default">
    <div class="panel-heading"><h1 class="panel-title"><i class="fa fa-pie-chart"></i> <?= $Lang->get('STATS__TITLE') ?></h1></div>
    <div class="panel-body">
